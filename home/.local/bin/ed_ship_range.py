...
def get_ship_stats():
    ...
    if reservoir_size == 0:
        logging.warning(
            f'Unknown reservoir size for {ship_name}, defaulting to 1t')

    base_mass = hull_mass + reservoir_size

    logging.debug(
        f'{hull_mass} + {reservoir_size} ({ship_name}) = {base_mass}')
    ...
    for mount, data in modules.items():
        ...
        exp_mod: str = data.get('specialModifications', None)
        ...
        if mount.endswith('Colour') or mount == 'VesselVoice' or mount.startswith('PaintJob') or mount.startswith('Decal') or mount.startswith('ShipName') or mount.startswith('ShipKit'):
            continue
        ...
        if module_mass == 0:
            ...
            if module_data['group_id'] == 93 or module_data['group_id'] == 80 or module_data['group_id'] == 81 or module_data['group_id'] == 90 or module_data['group_id'] == 142:
                pass
        ...
        if mount == 'FrameShiftDrive':
            ...
            if mods:
                optimal_mod = mods.get(
                    'OutfittingFieldType_FSDOptimalMass', None)
    ...
    result = {
        'cargo_cap': cargo_cap,
        'laden_ly': round(jump_equation(base_mass + fuel_cap + cargo_cap, min(fuel_cap,
                                                                              fsd_stats['maxfuel']), fsd_stats['optmass'], fsd_stats['fuelmul'], fsd_stats['fuelpower'], gfsdb_bonus), 2),
        'unladen_ly': round(jump_equation(base_mass + fuel_cap, min(fuel_cap,
                                                                    fsd_stats['maxfuel']), fsd_stats['optmass'], fsd_stats['fuelmul'], fsd_stats['fuelpower'], gfsdb_bonus), 2)
    }
...
def jump_equation(mass: float, fuel: float, fsdOpt: float, fsdMul: float, fsdExp: float, jmpBst: float):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--stat', choices=['cargo_cap', 'laden_ly', 'unladen_ly'])
